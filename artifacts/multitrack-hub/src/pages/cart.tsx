...
import { useGetCart, useRemoveFromCart, useApplyCoupon, useRemoveCoupon, useCreateOrder, getGetCartQueryKey } from "@workspace/api-client-react";
...
export default function Cart() {
  ...
  const handleRemove = (id: number) => {
    removeFromCart.mutate(
      { data: { productId: id } }, // Note: assuming productId for removal based on standard pattern
      {
        onSuccess: () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow container mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold tracking-tight mb-8">Meu Carrinho</h1>
        ...
        {!hasItems ? (
          <div className="flex flex-col items-center justify-center text-center py-24 bg-card/30 rounded-2xl border border-dashed border-border">
            ...
            <p className="text-muted-foreground max-w-md mb-8">
              Explore nosso catálogo e encontre as melhores multitracks para o seu repertório.
            </p>
            <Link href="/catalog">
              <Button size="lg" className="h-12 px-8 font-bold">Explorar Catálogo</Button>
            </Link>
          </div>
        ) : (
          <div className="grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-4">
              {cart.items.map((item) => (
                <div key={item.id} className="flex gap-4 p-4 bg-card border border-border rounded-xl relative group">
                  ...
                  {cart.couponCode && (
                    <div className="flex justify-between text-primary">
                      <span className="flex items-center gap-1">
                        ...
                        <button onClick={handleRemoveCoupon} className="ml-2 text-destructive hover:underline text-xs">Remover</button>
                      </span>
